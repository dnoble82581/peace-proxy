...
<div
	...
	<div
		...
		<div class="flex flex-col justify-end flex-1 p-4">
			<div
					class="space-y-1">
				@if($this->isPrivate)
					@foreach($this->room->messages->where('to_primary', true) as $message)
						@php
					...
					@endforeach
				@elseif($this->toTactical)
					@foreach($this->getFilteredMessages() as $message)
						@php
							$isOwnMessage = auth()->id() === $message->user_id;
							$isEmergent = $message->emergency;
						@endphp
	...
	<hr>
	<div x-data="{emergency: @entangle('emergency'), important: @entangle('important'), toPrimary: @entangle('toPrimary'), toTactical: @entangle('toTactical')}">
		<form
			...
			<div
					class=" dark:bg-gray-700 pl-6 flex items-center gap-4 pt-2"
					:class="{
						'bg-blue-50': toPrimary,
						'bg-green-50': toTactical && !toPrimary,
						'bg-gray-50': !toPrimary && !toTactical,
						'bg-indigo-50' : toPrimary && toTactical}">
				<button
				...
				<button
					...
					<svg
							...
							stroke="currentColor"
							class="size-6"
							:class="toTactical ? 'text-green-700' : ''">
						<path
								stroke-linecap="round"
								stroke-linejoin="round"
			...
			<div
					class="flex items-center px-3 py-2 dark:bg-gray-700"
					:class="{
						'bg-blue-50': toPrimary,
						'bg-green-50': toTactical && !toPrimary,
						'bg-gray-50': !toPrimary && !toTactical,
